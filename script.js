...
function validateForm() {
  ...
  const productCode = productCodeRegex.test(
    formulaire.elements["product-code"].value
  );
  const quantity = Number(formulaire.elements["quantity"].value) > 0;

  const complaintsChecked =
    complaintGroup.querySelectorAll('input[type="checkbox"]:checked').length >
    0;
  const complaintDescriptionValid = complaintGroup.querySelector(
    "#other-complaint:checked"
  )
    ? complaintDescription.querySelector("textarea").value.trim().length >= 20
    : true;

  const solutionSelected =
    desiredSolution.querySelectorAll('input[type="radio"]:checked').length > 0;
  const solutionDescriptionValid = desiredSolution.querySelector(
    "#other-solution:checked"
  )
      ...
      if (!formData[key]) {
        switch (key) {
          ...
          case "complaint-description":
            complaintDescription.querySelector("textarea").style.borderColor =
              "red";
            break;
          ...
          case "solution-description":
            solutionDescription.querySelector("textarea").style.borderColor =
              "red";
            break;
        }
      }
...
// Inputs simples
addChangeListener(
  formulaire.elements["full-name"],
  (input) => input.value.trim() !== ""
);
addChangeListener(formulaire.elements["email"], (input) =>
  emailRegex.test(input.value)
);
addChangeListener(formulaire.elements["order-no"], (input) =>
  orderNumRegex.test(input.value)
);
addChangeListener(formulaire.elements["product-code"], (input) =>
  productCodeRegex.test(input.value)
);
addChangeListener(
  formulaire.elements["quantity"],
...
radios.forEach((radio) => {
  radio.addEventListener("change", () => {
    const valid =
      desiredSolution.querySelectorAll('input[type="radio"]:checked').length >
      0;
